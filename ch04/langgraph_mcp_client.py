...
mcp_client: MultiServerMCPClient(
    {
        ...
        "weather": {
            # `python ch04/mcp_server/MCP_weather_server.py` 으로 MCP 서버를 먼저 실행함.
...
async def call_mcp_tools(state: AgentState) -> dict[str, Any]:
    """
    state["message"]를 포함한 AgentState를 받아 어떤 MCP 도구를 호출할 지 결정
    데모 목적으러, 사용자 메시지에 'weather'나 '날씨'가 포함되어 있으면 날씨 도구를 호출하고,
    수학 표현식(숫자/연산자)이 포함되어 있으면 수학 도구를 호출한다.
    그 외의 경우에는 기본 메시지를 반환한다.
    """
    messages = state["message"]
    last_msg = messages[-1].content.lower()

    # MCP_TOOLS를 전역 변수로 선언하여 한 번에 가져온다.
    global MCP_TOOLS
    if "MCP_TOOLS" not in globals():
        MCP_TOOLS = await mcp_client.get_tools()

    # 간단한 판단: 숫자/연산자 토큰이 있으면 "math"를, 'weather'나 '날씨'가 있으면 "weather"를 선택한다.
    # 실제로는 LLM이 적절한 도구를 판단하지만 여기서는 시연을 위해 간단하게 판단한다.
