#!/usr/bin/env python3
"""RLVR(Reinforcement Learning with Verifiable Rewards로 파인튜닝한 모델 테스트 스크립트

reinforcement_learning_with_verifiable_rewards.py로 학습한 Qwen2-0.5B 모델을 테스트 합니다.
IT 헬프데스크 도구 호출 방식(<tool_call>)을 생성합니다.
...
참고: GRPOTrainer는 save_strategy='ephch'으로 체크포인트를 저장합니다.
    output_dir에 config.json이 없으면 checkpoint-* 하위 디렉토리를 자동 탐색합니다.
"""
import argparse
import glob
import json
...
# RLVR 학습 데이터와 유사한 IT 헬프 데스크 프롬프트
DEFAULT_PROMPTS = [
...
def loadl_model(model_path: str):
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="테스트할 프롬프트 (미지성 시 기본 예시들 실행)"
    )
    parser.add_argument(
        "--model-path",
        type=str,
