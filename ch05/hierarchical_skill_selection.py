...
try:
    ...
    pass  # dotenv 없으면 그냥 넘어감

# OpenAI API 키 존재 여부 체크 (없으면 실행 중단)
...
# LLM 초기화
# temperature=0 → 항상 일관된(결정적) 응답을 얻기 위함
...
def query_wolfram_alpha(expression: str) -> str:
    ...
    # 사용자 입력을 URL-safe하게 인코딩해서 API 호출
    api_url = f'''https://api.wolframalpha.com/v1/result?i={requests.utils.quote(expression)}&appid=aiden.kim@bluedus.co.kr'''

    try:
        response = requests.get(api_url)  # GET 요청
        if response.status_code == 200:
            return response.text  # 결과 문자열 반환
        else:
...
def send_slack_message(channel: str, message: str) -> str:
    ...
    # Slack Bot Token 필요 (실제 사용 시 환경변수로 관리하는 것이 안전)
    headers = {
...
if not selected_group_name:
    ...
    if selected_group_name not in tool_groups:
        ...
        if not selected_tool:
            ...
            # 선택된 도구 호출 (LangChain tool invoke 방식)
            try:
